...
import { ACTOR_ID } from '../ApifyNaverAIOverview.node';
import { buildActorInput } from '../ApifyNaverAIOverview.properties';
...
/**
 * Shape a single AI Overview result according to the chosen Output mode.
...
function shapeItem(
	...
	// simplified
	const references = ((item.references ?? []) as Array<Record<string, any>>).map((r) => ({
		title: r.title,
		link: r.link ?? r.url,
	}));
	const relatedQuestions = ((item.related_questions ?? []) as Array<Record<string, any>>).map(
		(q) => q.question ?? q,
	);
	return {
		query: item.query,
		aiOverviewPresent: item.ai_overview_present,
		overview: item.markdown,
		references,
		relatedQuestions,
	};
}
